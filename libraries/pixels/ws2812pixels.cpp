...
WS2812Pixels::WS2812Pixels(uint16_t numPixels, int outputPin, uint8_t type) :
    Pixels(numPixels), outputPin(outputPin), type(type) {

    // Setup output
#ifdef __AVR__
    port    = portOutputRegister(digitalPinToPort(outputPin));
    pinMask = digitalPinToBitMask(outputPin);
#endif

    // Store offsets based on type
    if(type & NEO_GRB) { // GRB vs RGB; might add others if needed
        rOffset = 1;
        gOffset = 0;
        bOffset = 2;
    } else if (type & NEO_BRG) {
        rOffset = 1;
        gOffset = 2;
        bOffset = 0;
    } else if (type & NEO_RBG) {
        rOffset = 0;
        gOffset = 2;
        bOffset = 1;    
    } else {
